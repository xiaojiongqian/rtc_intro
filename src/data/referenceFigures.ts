  ...
  layeredEncoding: {
    ...
    figures: [
      ...
      },
      {
        ...
        badge: "空间层",
        caption: "720p 示例中，1280x720 约 1350 kbps、占 67%；640x360 约 450 kbps、占 22%；320x180 与 160x90 只占 8% 和 3%，但给弱网和小窗口提供兜底。",
        takeaways: [
          "高空间层最吃码率，只有大画面、主讲人或录制需要时才值得订阅。",
          "低空间层成本很小，却能让弱网端、小宫格和首帧可见性有兜底。",
          "上行预算不足时，先看空间层是否按 UI 布局和接收端能力订阅，而不是盲目提高总码率。",
        ],
        tone: "protocol",
      },
      {
        ...
        badge: "时间层码率",
        caption: "2 层时间层示例为 t0=0.6、t1=0.4；3 层为 0.4/0.3/0.3；4 层为 0.25/0.15/0.25/0.35。时间层把帧率收益拆成可丢弃的增量预算。",
        takeaways: [
          "降级不一定只能降分辨率，也可以先丢高时间层，用较少码率保住低帧率连续性。",
          "基础时间层 t0 必须稳定；如果 t0 都不稳，高时间层再多也无法修复观看体验。",
          "层数越多，SFU 控制越细，但编码、依赖管理和调度复杂度也会提高。",
        ],
        tone: "accent",
      },
      {
