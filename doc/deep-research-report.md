...
- AEC 需要远端参考信号抵消扬声器回灌，输出仍可能有 residual echo  
- AEC、AGC、噪声抑制直接影响可懂度，也可能误伤音乐、系统音频和高动态内容  
- 建议素材：AEC 流程图 + AEC 前后波形图  
- 学员互动：沿图指出近端语音、远端回放、麦克风混合信号和参考输入，再判断音乐教学是否应该默认强处理  
- 预计讲解：6 分钟  

**第 26 页｜视频编解码与参数选择**  
- 编码效率趋势只能说明方向，真实收益取决于内容、实现、preset、硬件路径和端侧功耗  
- RD 曲线说明同样分辨率下，不同内容达到同等质量所需码率差异很大  
- 分辨率、帧率、码率、QP、关键帧和层级要联动考虑  
- 建议素材：压缩效率趋势图 + 360p 不同场景 RD 曲线  
- 学员互动：让学生读曲线，判断会议人像、复杂运动、共享屏为什么不能共用一套码率/帧率策略  
- 预计讲解：6 分钟  
...
- SVC：同一编码流内部分层  
- SFU 借此做按需订阅与弱网适配  
- 空间层和时间层都需要码率预算，低层是兜底，高层服务大画面、流畅度和细节  
- 建议素材：Multicast/Simulcast/SVC 对比图 + 720p 空间层码率饼图 + 时间层码率配置 + SVC 时间层编号 + SFU 订阅/BWE 序列图  
- 学员互动：给出大画面/小宫格/弱网手机三类接收端，让学生选择应该订阅哪一层，并解释服务器如何合并订阅需求影响发布端上行  
- 预计讲解：6 分钟  
...
> 图示设计参考 `Juphoon-rtc-白皮书 .pdf` 中 MME 模块数据流、ARS 的 Sender/Receiver/RTP/TMMBR 反馈机制、弱网模块中的 FEC/RED/NACK/RTT/Loss/Jitter/Bandwidth 结构，以及 jitter buffer/PLC 的接收端数据连续性说明；网页 slides 已将经过筛选的示例图统一放入 `public/reference-figures/` 并通过 `referenceFigure` 图组承载，每张图只保留课堂观察点，避免原图信息过载。

**第 29 页｜带宽估计与拥塞控制**  
- 实时媒体拥塞控制不是把链路塞满，而是在每个 RTT 内重新选择可承受的体验  
- 接收端反馈要结合丢包、到达时间、延迟趋势、RTT 与 ECN，而不是只看 packet loss  
- 发送端动作包括编码码率、分层订阅、pacer 节奏和音视频优先级  
- 弱网时先保音频连续，再逐步降视频层、分辨率、帧率或码率  
- 建议素材：可用带宽与发送码率追踪图 + 全速/节省带宽模式曲线  
- 学员互动：让学生根据蓝线/黑线关系判断何时探测上升、何时快速回落、何时阶梯恢复  
- 预计讲解：6 分钟  

**第 30 页｜丢包恢复机制总览**  
- 先用一张数据流地图区分 NACK/RTX、PLI/FIR、FEC/RED、PLC：有的靠反向反馈，有的靠前向冗余，有的只在接收端补体验  
- NACK/RTX 解决具体 RTP 包缺口，前提是 RTT 与播放 deadline 允许  
- PLI/FIR 解决视频参考链或解码器刷新问题，不是普通丢包的默认动作  
- FEC/RED 用持续冗余换恢复时间；PLC 不恢复网络包，只降低用户感知断裂  
- 建议素材：发送端、网络、接收端、jitter buffer、播放时钟组成的丢包恢复总览图，明确反馈流、重传流、冗余流和本地补偿流  
- 学员互动：给出四个场景，让学生先把机制贴到链路位置，再判断该不该使用  
- 预计讲解：5 分钟  

**第 31 页｜NACK / RTX：包级重传**  
- NACK 的触发点是接收端发现 RTP sequence number 缺口  
- RTCP NACK 走反向反馈；RTX 或原 RTP 包走正向重传路径  
- 成败条件是“重传包是否能在 jitter buffer 播放 deadline 前到达”  
- 高 RTT、高丢包或发送队列已经拥塞时，重传会继续占用带宽预算  
- 建议素材：NACK/RTCP 请求与 Re-send 序列图  
- 学员互动：让学生用 RTT、buffer 深度和当前丢包率判断 NACK 是否值得发，并说明“补回来也来不及”的条件  
- 预计讲解：5 分钟  

**第 32 页｜PLI / FIR：画面刷新请求**  
- PLI 表示接收端发生 picture loss，当前视频参考链可能不可恢复  
- FIR 更偏向完整 decoder refresh，适合新接收端、源切换或解码器状态必须刷新  
- PLI/FIR 的反馈包很小，但触发的关键帧或刷新帧可能造成瞬时码率尖峰  
- 频繁 PLI/FIR 会形成关键帧风暴，拥塞时反而让链路更糟  
- 建议素材：FIR/RTCP 刷新序列图 + Key/P 参考链 + gold/alt 多参考帧 + 参考确认失败图  
- 学员互动：让学生区分“普通 RTP 包丢失”“持续花屏”“新端入会”分别应该先想 NACK、PLI 还是 FIR，并解释为什么关键帧要限频  
- 预计讲解：5 分钟  

**第 33 页｜FEC / RED：前向冗余**  
- FEC 在发送端预先生成冗余保护，接收端用主媒体包和冗余包重建缺口  
- RED 是 RTP 冗余封装思路，帮助主数据和冗余数据在负载结构中被识别  
- FEC 不等反馈，适合高 RTT、随机丢包和重传来不及的场景  
- FEC 的代价是持续带宽开销；10%–30% 冗余在拥塞链路上可能成为额外压力  
- 建议素材：FEC encode add packets / FEC decode 数据流图，突出主媒体包与冗余包同向传输  
- 学员互动：让学生判断“可用带宽不足”时为什么不应盲目打开高比例 FEC  
- 预计讲解：5 分钟  

**第 34 页｜PLC：丢包隐藏**  
- PLC 不恢复丢失的 RTP 包，而是在接收端生成可播放的替代片段  
- 音频 PLC 通常与 jitter buffer 配合：deadline 到了还没包，就用历史波形或语音连续性补一小段  
- PLC 适合短小、随机的缺口；连续长时间丢包会带来重复、发闷、机器人感或静音  
- 排查时要看 concealment events、concealedSamples、jitterBufferDelay 和音频可懂度，不要只看 packet loss  
- 建议素材：接收端 jitter buffer/PLC 拓扑图 + G.711 frame erasure concealment 波形图  
- 学员互动：让学生解释“丢包率不高但 concealedSamples 很高”为什么说明体验已经在靠补偿支撑  
- 预计讲解：5 分钟  

**第 35 页｜抖动缓冲与低时延优化**  
- 抖动缓冲把不稳定到达转换成稳定播放，但天然会增加端到端延迟  
- 过浅缓冲会丢播放 deadline，过深缓冲会让互动变迟钝  
- 关键指标：jitterBufferDelay、jitterBufferMinimumDelay、freeze/concealment、AV sync offset  
- 优化目标不是把 delay 归零，而是在当前网络下找到最小可用缓冲  
- 建议素材：RTP 到达、jitter buffer 队列、PLC/Decoder、播放时钟的数据流拓扑，配合过浅/平衡/过深三组曲线  
- 学员互动：让学生判断哪组 QoE 最差，并说明“稳定”和“实时”为什么不是同一个目标  
- 预计讲解：5 分钟  

**第 36 页｜回声消除、AGC 与噪声抑制**  
- 外放互动必须关注 AEC，否则远端会听到自己的声音回灌  
- 音乐、系统音频、伴奏和高动态内容不能盲目开强 NS/AGC  
- echoCancellation: "remote-only" 更偏向消除远端回放，"all" 更激进，会尝试移除系统产生的音频  
- 策略要和输入源、耳机/外放、共享系统音频和教学内容绑定  
- 建议素材：本地人声、远端回放、系统音频进入采集流后，经 AEC/NS/AGC、Opus、RTP 上行的拓扑图  
- 学员互动：分场景判断 AEC/AGC/NS 的默认策略，并说出“保护什么声音、消除什么声音”  
- 预计讲解：5 分钟  

**第 37 页｜互动：弱网恢复策略排序**  
- 场景：5% 丢包、RTT 220ms、视频冻结升高、音频仍可听  
- 要求学生根据不同 loss 区间排序 NACK+FIR、FEC+FIR、FIR、组合策略和多方会议策略  
- 教师参考方向：先稳住沟通，再压低视频负载，最后考虑会放大带宽压力或引入成本的动作  
- 建议素材：600kbps 下不同恢复策略随丢包率上升的帧率曲线  
- 学员互动：小组给出 1%、10%、30% 三个 loss 点的策略排序，并说明每条曲线在哪个区间开始失效  
- 预计讲解：6 分钟  

**第 38 页｜时延优化实战清单**  
- 用一条“进入房间 → 首帧编码 → RTP 发送 → 接收播放 → Stats 回写”的端到端序列讲低时延  
- 首帧等待先看权限、ICE、TURN；发送等待看采集、编码、队列和 pacer  
- 传输等待看 BWE、RTT、发送队列与拥塞队列；播放等待看 jitter buffer、解码队列和 AV sync  
- 所有动作必须回到 Stats 闭环，用指标判断是否真的改善 QoE，并保留回退条件  
- 建议素材：端到端序列图，覆盖建连、采集编码、发送网络、接收播放、观测闭环五段，并高亮当前等待点  
- 学员互动：逐段让学生指出“等待来自哪里、应该砍哪一段、用哪个指标证明”  
- 预计讲解：5 分钟  
...
- 录制、审核、AI 字幕和服务端诊断会与端到端加密产生能力冲突  
- 建议素材：服务端 RTP 录制拓扑 + 客户端/服务端录制路径图  
- 学员互动：沿录制拓扑标出明文媒体、RTP 文件、MP4 文件、转码和播放器位置，再辩论 E2EE 开启后录制应放在哪里  
- 预计讲解：6 分钟  
...
- 建议素材：混合接入拓扑 + JMP 媒体进程图 + PSTN room session 序列图 + DTMF 入会序列图  
- 学员互动：从每张图里标出控制面、媒体面、录制面，并说出排障时应该先看哪条证据  
- 预计讲解：6 分钟  

**第 41 页｜测试方法与工具**  
- `tc netem` 做受控网络扰动：延迟、抖动、丢包、限速和乱序分别注入  
- `getStats()` 看浏览器端真实统计，不只看服务器日志  
- `webrtc-bench` 做容量与回归测试，关注 SFU/TURN/编码负载和首帧成功率  
- RTCP XR 与 Wireshark 辅助包级证据分析，把端上现象和链路行为对齐  
- 建议素材：从网络扰动、端侧观测、容量回归到包级证据的测试工具链流程图  
- 学员互动：选择一个工具来验证“冻结率上升”的根因  
- 预计讲解：6 分钟  

**第 42 页｜getStats、RTCP XR 与监控公式**  
- QoS 指标看网络与媒体链路：RTT、packet loss、jitter、available bitrate  
- QoE 指标看用户体验：首帧时间、冻结、音频 concealment、A/V sync offset  
- 课堂中要求学生把公式写成脚本或仪表板，并定义健康区间和退化区间  
- 建议素材：监控看板示意，展示公式、样本值、健康/退化阈值和解释  
- 学员互动：现场计算一组简化 Stats，判断 QoS/QoE 是否退化  
- 预计讲解：6 分钟  

**第 43 页｜告警阈值与 SLO/SLA**  
- 阈值不是标准，而是业务化告警起点  
- 要按音频、视频、共享屏幕、人机语音分场景分级，不能一个阈值打所有场景  
- 好的 SLO 同时包含建连成功率、首帧、冻结、音频可懂度和降级动作  
- 建议素材：SLO 分层表，把业务承诺、指标、告警和降级动作放在同一行  
- 学员互动：为在线教育场景设计 3 条 SLO 和 1 条可执行降级策略  
- 预计讲解：6 分钟  

**第 44 页｜互动：线上故障复盘演练**  
- 现象：某区域 20 分钟内首帧时间和 TURN 回退率同时升高  
- 要求学生按客户端、DNS/调度、TURN/SFU、运营商、版本变更分层提出假设  
- 止损动作先恢复体验，再做根因确认：切区域、降级策略、扩容或回滚  
- 建议素材：事故时间线，逐步展示现象、证据、假设、动作和复盘沉淀  
- 学员互动：小组 3 分钟复盘，输出排查顺序、止损动作和下次更早发现的监控信号  
- 预计讲解：7 分钟  
...
- 建议素材：运行拓扑 + 步骤 rail，标出 WebSocket 信令、SRTP 媒体、STUN/TURN 候选路径  
- 学员互动：让每组预测浏览器会请求哪些权限、会产生哪些信令消息、最终媒体路径可能是什么  
- 预计讲解：4 分钟  

**第 46 页｜Node 信令服务器代码讲解**  
- 房间管理、offer/answer/candidate 透传  
- 信令只交换元数据，不承载媒体  
- 适合课堂第一段 live coding  
- 建议素材：join、offer、answer、candidate、close 五段代码 walkthrough，并同步展示转发方向  
- 学员互动：让学生标出状态管理、消息转发、生产安全守护分别在哪里  
- 预计讲解：7 分钟  

**第 47 页｜浏览器采集与 P2P 代码讲解**  
- `getUserMedia()`、`RTCPeerConnection`、`getStats()`  
- 观察 RTT、丢包、帧率、抖动缓冲时延  
- 适合做第一轮实验数据采集  
- 建议素材：采集、建 PC、协商、ICE、播放、Stats 的生命周期图，旁边配关键代码行  
- 学员互动：每组截取一组 Stats，并解释一个指标对应链路中的哪一段  
- 预计讲解：8 分钟  
...
- 建议素材：教师端 producer 上行一次、SFU router 按订阅向不同 consumer 下发的代码拓扑  
- 学员互动：让学生画出 producer 到 consumer 的数据流，并解释弱网学生为什么拿低层  
- 预计讲解：8 分钟  
...
- 建议素材：托管云、开源 SFU、自研协议栈、混合路线四象限生态地图  
- 学员互动：每组为一个假想产品选择托管、自建、自研或混合路线，并说明何时需要换路  
- 预计讲解：6 分钟  

**第 51 页｜2021–2026 标准、趋势与课程总结**  
- 标准更新：JSEP、SFrame、VP9 RTP、WHIP、WHEP、WebTransport  
- 趋势：边缘 SFU、AV1、AI RTC、直播入口/出口标准化  
- 未解决问题：E2EE 与服务端能力冲突、全球调度复杂度、监控语义统一  
- 建议素材：标准时间线 + 趋势张力卡 + 出口卡片  
...
| 25 | 音频编解码与处理 | AEC 参考输入与波形效果 | 6 分钟 | 图组观察 | 图示 |
| 26 | 视频编解码与参数选择 | 编码效率趋势与 RD 曲线 | 6 分钟 | 图组观察 | 图示 |
| 27 | Simulcast 与 SVC | 空间/时间层码率与 SFU 订阅 | 6 分钟 | 层级订阅 | 图示 |
| 28 | 互动：编解码取舍题 | 弱网移动端画质与功耗 | 6 分钟 | 方案对比 | 互动 |
| 29 | 带宽估计与拥塞控制 | 可用带宽、发送码率与节省带宽模式 | 6 分钟 | 曲线判断 | 图示 |
| 30 | 丢包恢复机制总览 | NACK/RTX、PLI/FIR、FEC/RED、PLC 的链路位置 | 5 分钟 | 机制贴图 | 图示 |
| 31 | NACK / RTX：包级重传 | RTP 序号缺口、RTCP NACK 与重传 deadline | 5 分钟 | 条件判断 | 图示 |
| 32 | PLI / FIR：画面刷新请求 | picture loss、decoder refresh 与关键帧尖峰 | 5 分钟 | 场景区分 | 图示 |
| 33 | FEC / RED：前向冗余 | 前向冗余、接收端 repair 与带宽成本 | 5 分钟 | 带宽取舍 | 图示 |
| 34 | PLC：丢包隐藏 | 接收端本地补偿、concealment 与体验兜底 | 5 分钟 | 指标解释 | 图示 |
| 35 | 抖动缓冲与低时延优化 | JBD 与低时延权衡 | 5 分钟 | 曲线判断 | 图示 |
| 36 | 回声消除、AGC 与噪声抑制 | 音频体验优化 | 5 分钟 | 场景策略 | 实验 |
| 37 | 互动：弱网恢复策略排序 | 不同策略在 loss 上升时的退化曲线 | 6 分钟 | 小组排序 | 图示 |
| 38 | 时延优化实战清单 | 端到端序列等待点和削减动作 | 5 分钟 | 时序诊断 | 序列图 |
| 39 | 安全与隐私 | 录制拓扑与明文边界 | 6 分钟 | 取舍辩论 | 图示 |
| 40 | 云、边缘与混合部署 | 混合接入、媒体进程与 PSTN/DTMF 序列 | 6 分钟 | 证据定位 | 图示 |
| 41 | 测试方法与工具 | netem、bench、stats、XR | 6 分钟 | 工具选择 | 实验 |
| 42 | getStats、RTCP XR 与监控公式 | 指标、公式、解释 | 6 分钟 | 现场计算 | 表格 |
| 43 | 告警阈值与 SLO/SLA | 指标分级与告警起点 | 6 分钟 | SLO 设计 | 表格 |
| 44 | 互动：线上故障复盘演练 | 区域故障排查 | 7 分钟 | 小组复盘 | 互动 |
| 45 | P2P 示例运行说明 | 环境、依赖、步骤 | 4 分钟 | 权限预测 | 代码 |
| 46 | Node 信令服务器代码讲解 | 房间与 SDP/ICE 交换 | 7 分钟 | 数据流标注 | 代码 |
| 47 | 浏览器采集与 P2P 代码讲解 | 采集、建连、getStats | 8 分钟 | 指标截图 | 代码 |
| 48 | SFU 多方会议代码讲解 | transport/producer/consumer | 8 分钟 | 数据流绘制 | 代码 |
| 49 | 实验复盘与课堂作业 | 弱网实验记录和解释 | 8 分钟 | 小组汇报 | 实验 |
| 50 | 行业格局与开源生态 | 商业与开源选型 | 6 分钟 | 路线选择 | 表格 |
| 51 | 2021–2026 标准、趋势与课程总结 | 标准演进、趋势、总结 | 8 分钟 | 出口卡片 | 图示 |
