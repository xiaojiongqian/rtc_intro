...
课堂建议按 **150–180 分钟**组织，适合“有一定网络和编程基础的工程师/学生”使用。比较稳妥的节奏是：前 45 分钟建立理论与协议框架，中 45 分钟讲清架构、编解码和性能优化，后 30–45 分钟讲安全、部署、测试与产业趋势，最后 30–45 分钟做两段代码讲解和一个网络扰动实验。互动环节建议放在第 12、20、28、33 页之后，问题可围绕“为什么 1 对 1 不一定该上 SFU”“为何 AV1 不一定适合所有终端”“什么指标最先预警质量下降”“为什么 TURN 不是坏事而是兜底”展开。评分建议采用“概念理解 30% + 实验完成度 40% + 指标解释能力 20% + 扩展思考 10%”。上述时间与评分为本课程建议配置。 
...
**第 1 页｜课程封面与学习目标**  
- 课程标题：RTC 实时通信核心技术、理论与行业发展  
- 学习目标：理解协议栈、架构、性能优化、安全与产业趋势  
- 交付物说明：报告、代码、图示、实验、阅读清单  
- 建议素材：一张“采集→传输→播放”链路图  
- 预计讲解：3 分钟  

**第 2 页｜课堂使用说明**  
- 时间分配：理论 90 分钟、代码 30 分钟、实验 30–60 分钟  
- 互动安排：4 次问答 + 1 次小组实验复盘  
- 评分方式：概念、代码、实验、分析四部分  
- 产出要求：实验记录、指标截图、简短反思  
- 建议素材：课程节奏表  
- 预计讲解：4 分钟  
...
**第 3 页｜RTC 是什么与为什么重要**  
- RTC 面向低时延双向交互，不等同于直播/VOD  
- 典型场景：会议、教育、连麦、客服、远程协作、AI 实时语音  
- 真正难点不只是“通了”，而是“稳、清、低时延、可扩展”  
- 建议素材：RTC 与直播/VOD 对比图  
- 预计讲解：4 分钟  

**第 4 页｜RTC 端到端链路**  
- 采集、预处理、编码、发送、网络传输、抖动缓冲、解码、渲染  
- 每个环节都可能贡献时延与质量损失  
- 统计与反馈必须贯穿全链路  
- 建议素材：端到端链路流程图  
- 预计讲解：5 分钟  
...
**第 5 页｜时延模型**  
- 端到端时延 = 采集 + 处理 + 编码 + 发送排队 + 网络 + 缓冲 + 解码 + 渲染  
- 交互系统中，稳定低时延通常比峰值码率更重要  
- 课堂中用“首帧时间”和“平均 RTT”做最直观指标  
- 建议素材：时延分解条形图  
- 预计讲解：5 分钟  
...
- 抖动是到达间隔波动，不是简单“延迟变大”  
- 同步涉及音视频对齐与多流对齐  
- `estimatedPlayoutTimestamp` 可近似观察 AV sync  
- 建议素材：音视频时钟对齐示意图  
- 预计讲解：5 分钟  

**第 7 页｜QoS 与 QoE**  
- QoS 更多关注网络层质量，QoE 更关注用户感知  
- 低丢包不一定代表体验好；深缓冲会“稳但不实时”  
- 工程目标是 QoS 与 QoE 的平衡，而不是单指标最优  
- 建议素材：QoS/QoE 关系图  
- 预计讲解：4 分钟  
...
**第 13 页｜DTLS、SCTP 与 DataChannel**  
- DTLS 负责安全握手与导出 SRTP 密钥  
- DataChannel 与媒体共享会话，可承载低时延控制/文本/状态同步  
- 课堂重点：媒体和数据并非必须走同一业务信令  
- 建议素材：DTLS-SRTP + DataChannel 示意图  
- 预计讲解：5 分钟  
...
**第 17 页｜音视频编解码全景**  
- 音频：Opus、G.711  
- 视频：VP8、H.264、VP9、AV1  
- 选型应围绕兼容性、编码成本、画质与场景做平衡  
- 建议素材：编解码矩阵表  
- 预计讲解：5 分钟  
...
| 页码 | 标题 | 要点摘要 | 预计讲解时长 | 是否含代码/实验/图示 |
|---|---|---|---:|---|
| 1 | 课程封面与学习目标 | 课程范围、目标、交付物 | 3 分钟 | 图示 |
| 2 | 课堂使用说明 | 时间、互动、评分、作业 | 4 分钟 | 表格 |
| 3 | RTC 是什么与为什么重要 | 定义、场景、难点 | 4 分钟 | 图示 |
| 4 | RTC 端到端链路 | 采集到播放全链路 | 5 分钟 | 图示 |
| 5 | 时延模型 | 时延分解与测量 | 5 分钟 | 图示 |
| 6 | 抖动、同步与播放时钟 | 抖动与 AV sync | 5 分钟 | 图示 |
| 7 | QoS 与 QoE | 网络质量与用户体验 | 4 分钟 | 图示 |
| 8 | 协议栈总览 | API、建连、安全、媒体 | 5 分钟 | 图示 |
| 9 | RTP、RTCP 与 SRTP | 媒体承载与反馈、安全 | 6 分钟 | 图示 |
| 10 | WebRTC API 与 JSEP | `RTCPeerConnection` 与状态机 | 6 分钟 | 图示 |
| 11 | SIP、SDP 与信令边界 | SIP 与自定义信令关系 | 5 分钟 | 图示 |
| 12 | ICE、STUN 与 TURN | NAT 穿透与中继回退 | 6 分钟 | 图示 |
| 13 | DTLS、SCTP 与 DataChannel | 安全握手与数据通道 | 5 分钟 | 图示 |
| 14 | P2P Mesh 架构 | 适用场景与缺点 | 5 分钟 | 图示 |
| 15 | SFU 架构 | 多方会议主流方案 | 6 分钟 | 图示 |
| 16 | MCU 与混合架构 | 混流与边缘/中心折中 | 5 分钟 | 图示 |
| 17 | 音视频编解码全景 | 编解码选型全局图 | 5 分钟 | 表格 |
| 18 | 音频编解码与处理 | Opus/G.711/AEC | 6 分钟 | 图示 |
| 19 | 视频编解码与参数选择 | VP8/H.264/VP9/AV1 | 6 分钟 | 表格 |
| 20 | Simulcast 与 SVC | 分层与多质量转发 | 6 分钟 | 图示 |
| 21 | 带宽估计与拥塞控制 | BWE、RTCP feedback | 6 分钟 | 图示 |
| 22 | 重传、NACK、PLI/FIR 与 FEC | 丢包恢复机制 | 5 分钟 | 图示 |
| 23 | 抖动缓冲与低时延优化 | JBD 与低时延权衡 | 5 分钟 | 图示 |
| 24 | 回声消除、AGC 与噪声抑制 | 音频体验优化 | 5 分钟 | 实验 |
| 25 | 时延优化实战清单 | 建连、媒体、观测优化 | 5 分钟 | 清单 |
| 26 | 安全与隐私 | SRTP/DTLS/SFrame/IP 隐私 | 6 分钟 | 图示 |
| 27 | 云、边缘与混合部署 | 部署模式与选型 | 6 分钟 | 图示 |
| 28 | 测试方法与工具 | netem、bench、stats、XR | 6 分钟 | 实验 |
| 29 | getStats、RTCP XR 与监控公式 | 指标、公式、解释 | 6 分钟 | 表格 |
| 30 | 告警阈值与 SLO/SLA | 指标分级与告警起点 | 5 分钟 | 表格 |
| 31 | P2P 示例运行说明 | 环境、依赖、步骤 | 4 分钟 | 代码 |
| 32 | Node 信令服务器代码讲解 | 房间与 SDP/ICE 交换 | 7 分钟 | 代码 |
| 33 | 浏览器采集与 P2P 代码讲解 | 采集、建连、getStats | 8 分钟 | 代码 |
| 34 | SFU 多方会议代码讲解 | transport/producer/consumer | 8 分钟 | 代码 |
| 35 | 行业格局与开源生态 | 商业与开源选型 | 6 分钟 | 表格 |
| 36 | 2021–2026 标准、趋势与课程总结 | 标准演进、趋势、总结 | 8 分钟 | 图示 |
